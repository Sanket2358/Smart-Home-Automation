...
def dashboard():
	...
	if(power<366):
		...
		load = 'Heavy Load'
	return render_template('dashboard.html',name=name,slot1=voltage,slot2=current,slot3=power,slot4=load)

@app.route('/controlHome', methods=['GET', 'POST'])
def controlHome():
	global name
	return render_template('controlHome.html',name=name)
...
def control():
	...
	# Update Firebase
	#ref = db.reference(f'appliances/{appliance}')
	#ref.set({'status': action})
	
	return jsonify({'message': f'{appliance.capitalize()} turned {action} successfully!'})
